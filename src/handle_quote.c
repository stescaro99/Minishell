...
void	handle_quote(char **tmp, int *i, char *l, t_echo **echo)
{
	int		flag;
	char	c;

	c = l[(*i) - 1];
	if ((!strncmp("echo ", &((*tmp)[ft_strlen(*tmp) - 5]), 5))
		|| (!strncmp("echo -n ", &((*tmp)[ft_strlen(*tmp) - 8]), 8)))
		return (echo_quotes(tmp, l, i, echo));
