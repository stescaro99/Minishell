...
/*   Updated: 2024/05/06 09:56:54 by stescaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_quote(char **t, int *i, char *l, t_echo **echo)
{
	...
	c = l[(*i) - 1];
	if ((SL(*t) > 7 && !strncmp("echo -n ", &((*t)[SL(*t) - 8]), 8))
		|| (SL(*t) > 4 && !strncmp("echo ", &((*t)[SL(*t) - 5]), 5)))
	...
	if (flag > 0)
		handle_limiter_or_filename(t, i, l, c);
	else if (flag == -1)
		handle_awk_marks(t, i, l, c);
	if (!(l[*i]) && l[*i -1] != c && flag >= 0 && ++(*i))
