...
/*   Updated: 2024/04/26 10:39:59 by stescaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	open_outfile(char *line, int *end, int fd_in)
{
	int		fd;
	char	*file;
	char	c;
...
int	pipe_shell(t_data *data, t_line **line)
{
	...
	if (fd_in < 0 && fd_in != -10)
		return (-1);
	fd_out = open_outfile((*line)->line, &end, fd_in);
	if (fd_out == -10)
		return (pipex_without_outfile(data, line, fd_in, start));
