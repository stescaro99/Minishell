...
/*   Updated: 2024/04/29 15:06:55 by stescaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	export_shell(t_env **env, t_line **line)
{
	...
	nm = ft_substr((*line)->line, 7, len_to_first_space((*line)->line) - 7);
	...
	{
		free(node->value);
		node->value = ft_strdup(&((*line)->line[7 + ft_strlen(nm)]));
		node->unsetted = 0;
	}
	return (handle_logic(line), 0);
}
