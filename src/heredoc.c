...
void	echo_quotes(char **tmp, char *l, int *i, t_memory **memory)
{
	int			end;
	char		c;
	char		*s;
	...
	if (c == '\"' && find_char(s, '$') >= 0 && ft_strlcat(*tmp, s, 25000))
		return (free(s));
	temp = get_memory_node(*memory, "echo quotes");
	str = ft_calloc(15000, 1);
	ft_strlcat(str, s, 15000);
	if (!odd_quotes(&l[*i], c))
		handle_odd_quotes(&str, 0, ft_strlen(str), c);
	if (temp && ft_free(temp->value))
		temp->value = ft_strdup(str);
	else
		memory_lstadd_back(memory, memory_lstnew("echo quotes", str));
