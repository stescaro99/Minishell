...
/*   Updated: 2024/05/03 13:26:46 by stescaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	echo_shell(t_line **l, int ret, t_memory *memory)
{
	int		i;
	char	*t;
	int		flag;
	...
	flag = 0;
	if (!t[i] || t[i + 1] == 14)
		return (echo_no_string(l, memory));
	if (t[++i] == '-' && t[i +1] && t[i +1] == 'n' && is_space(t[i +2]) && ++flag)
		i += 3;
	if (!t[i - 1] || !t[i])
		return (handle_logic(l), 0);
	while (t[i])
	{
		if (t[i] == '$' && !t[i + 1] && (*l)->next->line && (*l)->logic != -1)
			return (handle_logic(l), 0);
		else if (t[i] == '$' && t[i + 1] == '?')
			i += 0 * printf("%d", ret) + 2;
		else
	...
	(void)((flag) && printf("\e[38;2;0;0;0m\e[48;2;255;255;255m%%\033[0m"));
