...
/*   Updated: 2024/05/03 12:33:24 by stescaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*remove_marks(char *line, t_memory **memory, t_env *env)
{
	...
	{
		if ((line[i] == '\'' || line[i] == '\"') && ++i)
			handle_quote(&tmp, &i, line, memory);
		else
...
t_line	*line_lstnew(char *l, t_memory **memory, t_env *env)
{
	char	*tmp;
	t_line	*new;
	int		i;
